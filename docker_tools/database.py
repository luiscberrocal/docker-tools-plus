...
def get_cleanup_by_name(name: str) -> List[Cleanup]:
    ...
    try:
        with sqlite3.connect(settings.database_path) as conn:
            cur = conn.execute(
                "SELECT * FROM cleanups WHERE name LIKE ?",
                (f'%{name}%',)
            )
            return [Cleanup(**dict(zip(['id', 'name', 'regular_expression'], row))) for row in cur.fetchall()]
    except sqlite3.Error as e:
...
def create_cleanup(name: str, regex: str) -> Cleanup:
    ...
    try:
        with sqlite3.connect(settings.database_path) as conn:
            cur = conn.execute(
                "INSERT INTO cleanups (name, regular_expression) VALUES (?, ?)",
                (name, regex)
            )
            ...
            row = cur.fetchone()
            return Cleanup(**dict(zip(['id', 'name', 'regular_expression'], row)))
    except sqlite3.Error as e:
